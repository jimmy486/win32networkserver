	...
	{	
		...
		while (pThis->m_bThreadRun)
		{
			...
			{	
				...
				if (iterTcp != pThis->m_ContextMap.end())
				{
						...
						&& (pTcpContext->m_pSendContext->m_nPostCount <= 0))
					{
						...
						pThis->m_InvalidQue.erase(sock_iter);
						delete pTcpContext;
						pTcpContext = NULL;
			...
			for (map<SOCKET, TCP_CONTEXT*>::iterator map_iter = pThis->m_ContextMap.begin();
				pThis->m_ContextMap.end() != map_iter; )
			{
				//if (pThis->m_ContextMap.end() == pThis->m_MapIter)
				//{
				//	break;
				...
				SOCKET sock = map_iter->first;
				TCP_CONTEXT* pTcpContext = map_iter->second;
				...
				{
					pThis->m_ContextMap.erase(map_iter);
				}	
			}
