...
namespace HelpMng
{
   ...
   //             
	...
	TCP_CONTEXT::~TCP_CONTEXT()
	{
		...
		{
			TCP_SEND_DATA* pSendData = m_SendDataQue.front();
			m_SendDataQue.pop();
			if (NULL != pSendData)
			{
				delete pSendData;
	...
	void CTcpMng::PushInInvalidQue(const SOCKET& s)
	{
		try
		{
			...
			const bool bEmpty = m_InvalidQue.empty();
...
#define ACCEPT_NUM 10
			for (INT index = 0; index < ACCEPT_NUM; )
			{
				...
				nRet = s_pfAcceptEx(m_hSock, clientSock, pAccContext->m_pBuf, 0
					, sizeof(sockaddr_in) +16, sizeof(sockaddr_in) +16, NULL, &(pAccContext->m_ol));
	...
	void CTcpMng::AcceptCompletionProc(DWORD dwErrorCode, DWORD dwNumberOfBytesTransfered, LPOVERLAPPED lpOverlapped)
	{
		...
		try
		{
			...
			else
			{
				...
				pTcpContext = new TCP_CONTEXT(pContext->m_pMng);
				//系统可能已经没有足够的内存可以使用
				if (NULL == pTcpContext)
				{
					closesocket(pContext->m_hRemoteSock);
				...
				else
				{
					...
					{					
						...
						iErrCode = WSARecv(pTcpContext->m_pRcvContext->m_hSock, &RcvBuf, 1
							, NULL, &nFlag, &(pTcpContext->m_pRcvContext->m_ol), NULL);
			...
			iErrCode = s_pfAcceptEx(pContext->m_hSock, clientSock, pContext->m_pBuf, 0
				, sizeof(sockaddr_in) +16, sizeof(sockaddr_in) +16, NULL, &(pContext->m_ol));
			//accept发生错误
	...
	{	
		...
		while (pThis->m_bThreadRun)
		{
			...
			for (; (nCount < QUE_SIZE) && (nCount < MAX_COUNT); pThis->m_InvalidIter++, nCount++)
			{	
				//已到达队列的尾部，结束本次遍历
				if (pThis->m_InvalidQue.end() == pThis->m_InvalidIter)
				{
				...
				if (iterTcp != pThis->m_ContextMap.end())
				{
					TCP_CONTEXT* pTcpContext = iterTcp->second;

					if ((NULL != pTcpContext)
						&& (pTcpContext->m_pRcvContext->m_nPostCount <= 0)
						&& (pTcpContext->m_pSendContext->m_nPostCount <= 0))
					{
						pThis->m_ContextMap.erase(sInvalid);					
			...
			for (nCount = 0; (nCount < CONTEXT_NUM && nCount < MAX_COUNT); pThis->m_MapIter++, nCount++)
			{
				if (pThis->m_ContextMap.end() == pThis->m_MapIter)
				{
					break;
		...
		try 
		{
			...
			if (m_ContextMap.end() != iterMap)
			{
				...
				if (pContext)
				{
					...
					if (bSend)
					{
						INT iErr = WSASend(pContext->m_pSendContext->m_hSock, &SendBuf, 1, NULL, 0, &(pContext->m_pSendContext->m_ol), NULL);
